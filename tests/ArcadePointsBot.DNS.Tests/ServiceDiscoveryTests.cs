...
public class ServiceDiscoveryTests
{
    ...
    public void Discover_AllServices()
    {
        ...
        try
        {
            sd.Advertise(service);
            mdns.Start();
            Assert.True(done.WaitOne(TimeSpan.FromSeconds(1)), "DNS-SD query timeout");
        }
        finally
        {
            sd.Dispose();
    ...
    public void Discover_ServiceInstance()
    {
        ...
        try
        {
            sd.Advertise(service);
            mdns.Start();
            Assert.True(done.WaitOne(TimeSpan.FromSeconds(1)), "instance not found");
        }
        finally
        {
            sd.Dispose();
    ...
    public void Discover_ServiceInstance_WithAnswersContainingAdditionRecords()
    {
        ...
        Assert.True(done.WaitOne(TimeSpan.FromSeconds(3)), "instance not found");
        ...
        Assert.Empty(discovered!.AdditionalRecords);
        Assert.Equal(answersCount, discovered.Answers.Count);
    }
    ...
    public void Unadvertise()
    {
        ...
        try
        {
            ...
            Assert.True(done.WaitOne(TimeSpan.FromSeconds(1)), "goodbye timeout");
        }
        finally
        {
            sd.Dispose();
    ...
    public void ReverseAddressMapping()
    {
        ...
        try
        {
            ...
            {
                Assert.Equal(arpaAddress, answer.Name);
                Assert.True(answer.TTL > TimeSpan.Zero);
                Assert.Equal(DnsClass.IN, answer.Class);
            }
        }
        finally
        {
    ...
    public void Announce_ContainsSharedRecords()
    {
        ...
        try
        {
            ...
            mdns.Start();
            Assert.True(done.WaitOne(TimeSpan.FromSeconds(3)), "announce timeout");
        }
        finally
        {
            mdns.Stop();
    ...
    public void Announce_SentTwice()
    {
        ...
        try
        {
            ...
            mdns.NetworkInterfaceDiscovered += (s, e) => sd.Announce(service);
            mdns.Start();
            Assert.True(done.WaitOne(TimeSpan.FromSeconds(4)), "announce timeout");
        }
        finally
        {
            mdns.Stop();
