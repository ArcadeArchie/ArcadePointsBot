using ArcadePointsBot.DNS.Multicast.Events;
using System;
...
public class MulticastService : IResolver, IDisposable
{
    ...
    static readonly IPNetwork[] LinkLocalNetworks = [IPNetwork.Parse("169.254.0.0/16"), IPNetwork.Parse("fe80::/10")];
    ...
    /// </summary>
    private readonly RecentMessages sentMessages = new();

    /// <summary>
    ///   Recently received messages.
    /// </summary>
    private readonly RecentMessages receivedMessages = new();
    ...
    /// </summary>
    private readonly UdpClient unicastClientIp4 = new(AddressFamily.InterNetwork);

    /// <summary>
    ///   Use to send unicast IPv6 answers.
    /// </summary>
    private readonly UdpClient unicastClientIp6 = new(AddressFamily.InterNetworkV6);
    ...
    void FindNetworkInterfaces()
    {
        ...
        {
            Log.Logger.Error(e, "FindNics failed");
        }
    }
    ...
    public void OnDnsMessage(object? sender, UdpReceiveResult result)
    {
        ...
        {
            Log.Logger.Warning(e, "Received malformed message");
        ...
        {
            Log.Logger.Error(e, "Receive handler failed");
            // eat the exception
        }
    }
    ...
    public Task<Message?> ResolveAsync(Message request, CancellationToken cancel = default)
    {
        ...
        void checkResponse(object? s, MessageEventArgs e)
        {
            ...
            if (request.Questions.All(q => response?.Answers.Any(a => a.Name == q.Name) ?? false))
            {
